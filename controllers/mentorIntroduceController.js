...
//버튼 클릭 시 화면
    const getMentorIntroduce = async(req,res)=>{
        ...
        try{
            const mentor_intro = await MentorIntroduce.findOne({mentor_id :mentor_id});
    ...
    const createUser = async(req,res) => {
        try {
            ...
            res.status(201).json({
                message: "멘토 포스팅 작성 성공",
                data: saveUser,
              });
              console.log("멘토 포스팅 작성 성공");
        }catch (error) {
            console.error("멘토 포스팅 작성 실패 : ", error);
            res.status(500).json({ error: "포스팅을 생성하는 도중 오류가 발생했습니다." });
          }
